...
fn do_cc() {
    ...
    if cfg!(unix) && !target.contains("wasi") && !target.contains("minix") {
...
fn do_ctest() {
    match &env::var("TARGET").unwrap() {
        ...
        t if t.contains("minix") => return test_netbsd(t),
...
fn test_netbsd(target: &str) {
    assert!(target.contains("netbsd") || target.contains("minix"));
